...
static public class NGUITools
{
	...
	static public AudioSource PlaySound (AudioClip clip, float volume, float pitch)
	{
		...
		volume *= GameDefines.Setting_SoundVol * 0.01f;
	...
	static public void SetTweenActive(GameObject go, bool state,iTween.EventDelegate onActiveFinished = null) 
	{ 
		TweenPosition[] tweenPostions = go.GetComponentsInChildren<TweenPosition>();
		if (onActiveFinished != null)
		{
			if (state == true)
			{
				SetActive(go, state, true); 
				tweenPostions = go.GetComponentsInChildren<TweenPosition>();
				TweenPosition onListenTween = null;
				for (int i=0; i<tweenPostions.Length; i++)
				{
					if (tweenPostions[i].tweenGroup == 0)
					{
						tweenPostions[i].ResetToBeginning();
						tweenPostions[i].Play();
						onListenTween = tweenPostions[i];
					}
				}
				if (onListenTween != null)
				{
					EventDelegate.Add( onListenTween.onFinished , delegate{
						if (onActiveFinished != null)
							onActiveFinished();
					},true);
				}
			...
			else{///当时设置不可见的时候要取groudp为1的 tweenposition//
				TweenPosition onListenTween = null;
				for (int i=0; i<tweenPostions.Length; i++)
				{
					if (tweenPostions[i].tweenGroup == 1)
					{
						tweenPostions[i].ResetToBeginning();
						tweenPostions[i].Play();
						onListenTween = tweenPostions[i];
	...
	static public void TweenScale(GameObject go, bool state,iTween.EventDelegate onActiveFinished = null) 
	{ 
		TweenScale[] tweenScales = go.GetComponentsInChildren<TweenScale>();
		if (onActiveFinished != null)
		{
			if (state == true)
			{
				SetActive(go, state, true); 
				tweenScales = go.GetComponentsInChildren<TweenScale>();
				TweenScale onListenTween = null;
				for (int i=0; i<tweenScales.Length; i++)
				{
					if (tweenScales[i].tweenGroup == 0)
					{
						tweenScales[i].ResetToBeginning();
						tweenScales[i].Play();
						onListenTween = tweenScales[i];
					}
				}
				if (onListenTween != null)
				{
			...
			else{///当时设置不可见的时候要取groudp为1的 tweenScales//
				TweenScale onListenTween = null;
				for (int i=0; i<tweenScales.Length; i++)
				{
					if (tweenScales[i].tweenGroup == 1)
					{
						tweenScales[i].ResetToBeginning();
						tweenScales[i].Play();
						onListenTween = tweenScales[i];
	...
	static public void TweenRotations(GameObject go, bool state,iTween.EventDelegate onActiveFinished = null) 
	{ 
		TweenRotation[] tweenRotations = go.GetComponentsInChildren<TweenRotation>();
		if (onActiveFinished != null)
		{
			if (state == true)
			{
				SetActive(go, state, true); 
				tweenRotations = go.GetComponentsInChildren<TweenRotation>();
				TweenRotation onListenTween = null;
				for (int i=0; i<tweenRotations.Length; i++)
				{
					if (tweenRotations[i].tweenGroup == 0)
					{
						tweenRotations[i].ResetToBeginning();
						tweenRotations[i].Play();
						onListenTween = tweenRotations[i];
					}
				}
				if (onListenTween != null)
				{
				...
				for (int i=0; i<tweenRotations.Length; i++)
				{
					if (tweenRotations[i].tweenGroup == 1)
					{
						tweenRotations[i].ResetToBeginning();
						tweenRotations[i].Play();
						onListenTween = tweenRotations[i];
