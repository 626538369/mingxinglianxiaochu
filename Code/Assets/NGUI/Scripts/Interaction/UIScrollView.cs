...
public class UIScrollView : MonoBehaviour
{
	...
	public bool ShouldMoveEndLeft = false;
	public bool ShouldMoveEndRight = false;
	public bool ShouldMoveEndUp = false;
	public bool ShouldMoveEndDown = false;
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			else
			{
				...
				if (!canMoveHorizontally) constraint.x = 0f;
				if (!canMoveVertically) constraint.y = 0f;
				...
				if (constraint.sqrMagnitude > 1f)
				{
					if (dragEffect == DragEffect.MomentumAndSpring)
					{
						...
						if(!canMoveVertically)
						{
							if(constraint.x > 0)
							{
								ShouldMoveEndRight = true;
							}else
							{
								ShouldMoveEndLeft = true;
							}
						}
						...
						if(!canMoveHorizontally)
						{
							if(constraint.y > 0)
							{
								ShouldMoveEndUp = true;
							}else
							{
								ShouldMoveEndDown = true;
							}
						}
					}
				}
	...
	public void SetShouldMoveEnd()
	{
		ShouldMoveEndLeft = false;
		ShouldMoveEndRight = false;
		ShouldMoveEndUp = false;
		ShouldMoveEndDown = false;
	}
