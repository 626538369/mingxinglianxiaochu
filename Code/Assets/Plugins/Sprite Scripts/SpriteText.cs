...
public class SpriteText : MonoBehaviour, IUseCamera 
{
	...
	// tzz added for GUIWindowForm animation
	[HideInInspector]
	public float animColorAlpha		= 1.0f;
	
	/// <summary>
	/// The color of the sea client project
	/// </summary>
	public SeaClientColorType seaColor = SeaClientColorType.NothingColor;
	...
	/// <summary>
	/// The special color of the underline.
	/// </summary>
	public Color underlineColor = new Color(1.0f,1.0f,0.332f,1.0f);
	
	/// <summary>
	/// The underline axes list.
	/// </summary>
	private List<Vector3> m_underlineList = new List<Vector3>();
	
	/// <summary>
	/// The underline in one line
	/// </summary>
	private bool		m_underlineInOne		= true;
	
	/// <summary>
	/// Tag color collider.
	/// </summary>
	public class TagColorCollider{
		private BoxCollider			collider	= null;
		private GameObject			go			= null;
		private object				gameData	= null;
		
		public TagColorCollider(){
			go 			= new GameObject("SpriteTagCollider");
		...
		public BoxCollider GameObjCollider{
			get{return collider;}
		}
	...
	//! whether has tags collider to trigger text clicking event
	[HideInInspector] public bool m_hasColorTagCollider = false;
	
	//! collider list 
	private List<TagColorCollider>	m_tagColorColliderList 	= null;
	private List<Color> 	m_tagColorCreateList = null;
	private Color 			m_tagColorFormerColor 			= Color.clear;
	private Vector3 		m_tagColorFormerUpperleft		= Vector3.zero;
	private float			m_tagColorTotalHeightMin		= 0;
	private TagColorCollider m_tagColorCurrCollider 		= null;
	private bool			m_tagColorCreateColliderState	= false;
	
	//! the delegate for tag color collider
	public delegate void TagColorColliderTestCollider(SpriteText _spriteText,TagColorCollider _tagCollider);
	
	//! the delegate test
	private TagColorColliderTestCollider m_tagColorColliderTest = null;
	
	//! set the tag Color collider test delegate 
	public void SetTagColorColliderDelegate(TagColorColliderTestCollider _delegate){
		m_tagColorColliderTest = _delegate;
	}
	...
	/// <summary>
	/// The word config for sea client
	/// </summary>
	public int	wordConfig = 0;
	...
	//! hxl added for teach hide obj
	private bool mTeachRenderEnabled  = false;
	public bool TeachRenderEnabled
	{
		get{return mTeachRenderEnabled;}
		set{mTeachRenderEnabled = value;}
	}
		...
		if(colorTags.Count < 1)
		{
			...
			if(maxWidth > 0 && multiline)
			{
				...
				for(int i=0; i<str.Length; ++i)
				{
					...
					if (spaceLeft < 0)
					{
						// tzz modified for adapter Chinese characters
						//
						displaySB.Append('\n');
...
//						}
			...
			Color tFinalColor = GetSeaClientColor();
			
			for (int i = 0; i < colors.Length; ++i)
				colors[i] = tFinalColor;
...
*/
		...
		tempColor = GetSeaClientColor(); // Use our global color by default
	...
	/// <summary>
	/// Gets the color of the sea client.
	/// </summary>
	/// <returns>
	/// The sea client color.
	/// </returns>
	private Color GetSeaClientColor(){
		
		if(seaColor != SeaClientColorType.NothingColor){
			
			float tRed		= (((int)seaColor >> 16) & 0xff) / 256.0f;
			float tGreen	= (((int)seaColor >> 8) & 0xff) / 256.0f;
			float tBlue		= ((int)seaColor & 0xff) / 256.0f;
			
			return new Color(tRed,tGreen,tBlue,color.a);
	...
	protected Color ParseColor(string str)
	{
		...
		return GetSeaClientColor() * new Color(float.Parse(parts[0]), float.Parse(parts[1]), float.Parse(parts[2]), float.Parse(parts[3]));
	}
	...
	{
		//! tzz added for underline
		if(str == "u"){
			return new Color(underlineColor.r,underlineColor.g,underlineColor.b,underlineColor.a * color.a);
		}
		
		if(str.Length < 6)
			return GetSeaClientColor();
		...
		try
		{
			...
			return new Color(((float)red) / 255f, ((float)green) / 255f, ((float)blue) / 255f, ((float)alpha) / 255f * color.a);
		}
		catch
		{
			return GetSeaClientColor();
	...
	/// <summary>
	/// tzz added for Updates the underline mesh .
	/// </summary>
	private void UpdateUnderline(){
		
		if(m_underlineList.Count == 0){
			
			// tzz added if this underLine has been clipped
			int t_num = displayString.Length * 4;
			if(vertices.Length > t_num){				
				for(int i = 0;i <  vertices.Length - t_num; i++){
					int t_idx = i + t_num;					
					vertices[t_idx].x = vertices[t_idx].y = vertices[t_idx].z = 0;
				}
			}			
			return;			
		}
				
		Vector3[] t_underlineList 		= m_underlineList.ToArray();
		
		int vNum = t_underlineList.Length * 4 + capacity * 4;
		if(vertices.Length > vNum){
			vNum = vertices.Length;
		}
			
		Vector3[] t_underLineVertices	= new Vector3[vNum];
		Vector2[] t_underLineUVs		= new Vector2[vNum];
		Color[]	t_underLineColors		= new Color[vNum];
		int[]	t_underLineFace			= new int[t_underlineList.Length * 6 + capacity * 6];
				
		//copy to the underline data
		System.Array.Copy(vertices,t_underLineVertices,vertices.Length);
		System.Array.Copy(meshColors,t_underLineColors,meshColors.Length);
		System.Array.Copy(UVs,t_underLineUVs,UVs.Length);
				
		// Setup underline faces:
		for (int i = 0; i < t_underlineList.Length + capacity; ++i)
		{
			// Clock-wise winding:
			t_underLineFace[i * 6 + 0] = i * 4 + 0;	//	0_ 1			0 ___ 3
			t_underLineFace[i * 6 + 1] = i * 4 + 3;	//  | /		Verts:	 |	/|
			t_underLineFace[i * 6 + 2] = i * 4 + 1;	// 2|/				1|/__|2

			t_underLineFace[i * 6 + 3] = i * 4 + 3;	//	  3	
			t_underLineFace[i * 6 + 4] = i * 4 + 2;	//   /|
			t_underLineFace[i * 6 + 5] = i * 4 + 1;	// 5/_|4
		}
		
				
		float t_height 		= characterSize * 0.1f;
		float t_UVOffset	= 0.002f;
		SpriteChar ch		= spriteFont.GetSpriteChar('_');
		Color tUnderLineColor = GetChannelColor(underlineColor,ch);
		
		for(int i = 0,vertNum = capacity * 4;i < t_underlineList.Length;i++,vertNum += 4){
			
			// 			0 ___ 3
			// Verts:	 |	/|
			// 			1|/__|2
			
			Vector3	underline = t_underlineList[i];
						
			t_underLineVertices[vertNum + 0].x = underline.x;
			t_underLineVertices[vertNum + 0].y = underline.z;
			
			t_underLineVertices[vertNum + 1].x = underline.x;
			t_underLineVertices[vertNum + 1].y = underline.z - t_height;
						
			t_underLineVertices[vertNum + 2].x = underline.y;
			t_underLineVertices[vertNum + 2].y = underline.z - t_height;
			
			t_underLineVertices[vertNum + 3].x = underline.y;
			t_underLineVertices[vertNum + 3].y = underline.z;
			
			
				
			t_underLineColors[vertNum + 0] = tUnderLineColor;
			t_underLineColors[vertNum + 1] = tUnderLineColor;
			t_underLineColors[vertNum + 2] = tUnderLineColor;
			t_underLineColors[vertNum + 3] = tUnderLineColor;
			
			t_underLineUVs[vertNum].x = ch.UVs.x + t_UVOffset;
			t_underLineUVs[vertNum].y = ch.UVs.yMax;
	
			t_underLineUVs[vertNum + 1].x = ch.UVs.x + t_UVOffset;
			t_underLineUVs[vertNum + 1].y = ch.UVs.y;
			
			t_underLineUVs[vertNum + 2].x = ch.UVs.xMax - t_UVOffset;
			t_underLineUVs[vertNum + 2].y = ch.UVs.y;
			
			t_underLineUVs[vertNum + 3].x = ch.UVs.xMax - t_UVOffset;
			t_underLineUVs[vertNum + 3].y = ch.UVs.yMax;
		}
				
		vertices	= t_underLineVertices;
		meshColors	= t_underLineColors;
		UVs			= t_underLineUVs;
		faces		= t_underLineFace;		
	}
	...
	/// I char.
	/// </param>
	private Color GetChannelColor(Color iColor,SpriteChar iChar){
		
		if(iChar.channel == 0xf){
			return iColor;
		}
	...
	public void UpdateMesh()
	{
		...
		// tzz added for tag color collider
		if(m_hasColorTagCollider && m_tagColorColliderList == null){
			m_tagColorColliderList = new List<TagColorCollider>(); 
		}
...
// 			}
			...
			// tzz added for update underline
			UpdateUnderline();
		}
	...
	/** tzz added 
	 * get the tag color collider list for testing
	 */ 
	public List<TagColorCollider> GetTagColorColliderList(){
		return m_tagColorColliderList;
	}
	
	/** tzz added
	 * get the tag color create list for judge which color will create collider
	 */ 
	public List<Color> GetTagColorCreateList(){
		if(m_tagColorCreateList == null){
			m_tagColorCreateList	= new List<Color>();
		}
		return m_tagColorCreateList;
	}
	//@}}
...
// 					lineStart = curNewLine + 1;
	...
	// of a character.
	protected void BuildCharacter(int vertNum, int charNum, Vector3 upperLeft, ref SpriteChar ch,Vector3 startPos)
	{
		// 			0 ___ 3
		// Verts:	 |	/|
		// 			1|/__|2
		
		upperLeft += new Vector3(0,-2,0);
		...
		//------- Colors --------
		Color tCharColor = GetChannelColor(colors[charNum],ch);
		meshColors[vertNum] = tCharColor;
		meshColors[vertNum + 1] = tCharColor;
		meshColors[vertNum + 2] = tCharColor;
		meshColors[vertNum + 3] = tCharColor;
		
		bool t_vertClipped = false;
		...
		// tzz added to add underline axes
		if(colors[charNum] == underlineColor){
			
			if(!t_vertClipped){
				
				if(charNum > 0 && colors[charNum - 1] == underlineColor && m_underlineInOne){
					
					m_underlineList[m_underlineList.Count - 1] = new Vector3(m_underlineList[m_underlineList.Count - 1].x,
																			vertices[vertNum + 2].x,
																			m_underlineList[m_underlineList.Count - 1].z);
				}else{
					
					m_underlineList.Add(new Vector3(vertices[vertNum + 1].x,vertices[vertNum + 2].x,vertices[vertNum + 1].y));
					m_underlineInOne = true;
				}	
			}
			
		}else{
			m_underlineInOne = false;
		}
		
		// tzz added for add tag color colliders
		if(m_tagColorCreateColliderState && m_hasColorTagCollider && m_tagColorCreateList != null){
					
			if(m_tagColorFormerColor != colors[charNum]){
				m_tagColorFormerColor = colors[charNum];
								
				bool t_createCollider = false;
				foreach(Color c in m_tagColorCreateList){
					if(c == m_tagColorFormerColor){
						t_createCollider = true;
						break;
					}
				}
				...
				if(t_createCollider){
					// new a collider
					m_tagColorCurrCollider = new TagColorCollider();					
					m_tagColorColliderList.Add(m_tagColorCurrCollider);
					
					m_tagColorCurrCollider.GameObj.transform.parent 		= transform;
					m_tagColorCurrCollider.GameObj.transform.localPosition 	= Vector3.zero;
					
					m_tagColorFormerUpperleft = upperLeft;
				}else{
					m_tagColorCurrCollider = null;
				}
			}
			
			if(m_tagColorCurrCollider != null){
				
				if(m_tagColorTotalHeightMin > vertices[vertNum + 1].y){
					m_tagColorTotalHeightMin = vertices[vertNum + 1].y;
				}

				float t_width	= vertices[vertNum + 2].x - m_tagColorFormerUpperleft.x;
				float t_height	= startPos.y - m_tagColorTotalHeightMin;
				
				float t_x		= m_tagColorFormerUpperleft.x + t_width * 0.5f;
				float t_y		= startPos.y - t_height * 0.5f;
						
				m_tagColorCurrCollider.GameObjCollider.size 			= new Vector3(t_width,t_height,upperLeft.z);
				m_tagColorCurrCollider.GameObj.transform.localPosition	= new Vector3(t_x,t_y,m_tagColorFormerUpperleft.z);
			}
		}
	}
	...
	protected void Layout_Line(Vector3 startPos, string txt, int charIdx)
	{
		...
		// tzz added for collider of color tag
		if(m_hasColorTagCollider){
			m_tagColorCurrCollider 		= null;
			m_tagColorFormerColor 		= GetSeaClientColor();
			m_tagColorTotalHeightMin	= 100000;
			m_tagColorFormerUpperleft 	= startPos;
		}
		
		//! next line to break underline
		m_underlineInOne = false;
		...
		if (ch != null)
		{
			...
			BuildCharacter(charIdx * 4, charIdx, pos, ref ch,startPos);
		}
		...
		for (int i = 1; i < txt.Length; ++i)
		{
			...
			pos = startPos + new Vector3(ch.xOffset * worldUnitsPerTexel, ch.yOffset * worldUnitsPerTexel, 0);
			BuildCharacter((charIdx + i) * 4, charIdx + i, pos, ref ch,startPos);
		}
	}
	...
	protected void ClearMesh()
	{
		...
		for(int i=0; i<vertices.Length; ++i)
		{
			...
			meshColors[i] = GetSeaClientColor();
		}
...
//			gameObject.active = ((Component)parentControl).gameObject.active;
	...
	/// <summary>
	/// Gets or sets the color of the sea and refresh the text
	/// </summary>
	/// <value>
	/// The color of the sea.
	/// </value>
	public SeaClientColorType SeaColor
	{
		get { return seaColor;}
		set {
			seaColor = value;
	...
	//! tzz added for get the height of mesh
	public float GetHeight(){
		if(lines == null ){
			return lineSpaceSize;
		}else{
			return lineSpaceSize * lines.Length;	
		}		
	}
	...
	public string Text
	{
		...
		{			
			...
			// tzz added for clear the tag color collider
			if(m_tagColorColliderList != null){			
				foreach(TagColorCollider c in m_tagColorColliderList){
					if(Application.isEditor){
						DestroyImmediate(c.GameObj);
					}else{
						Destroy(c.GameObj);
					}
					
				}
				m_tagColorColliderList.Clear();
			}
			
			// clear the underlineList
			m_underlineList.Clear();
			
			// UpdateMesh will called in LasterUpdate that state would not create collider
			m_tagColorCreateColliderState = true;
				
			UpdateMesh();
			
			m_tagColorCreateColliderState = false;
		}
	}
	...
	//! tzz added for color tag collider
	private void ColorTagColliderTest(){
	
		if(!m_hasColorTagCollider || m_tagColorColliderTest == null){
			return ;
		}
		
		bool t_clicked  = false;		
		Vector2 t_pos	= Vector2.zero;
		
		if( Application.isEditor ){
			t_clicked 	= Input.GetMouseButtonDown(0);
			if(t_clicked){
				t_pos		= new Vector2(Input.mousePosition.x,Input.mousePosition.y);
			}
		}else{
#if UNITY_IPHONE || UNITY_ANDROID
			t_clicked	= Input.touches[0].phase == TouchPhase.Began;
			if(t_clicked){
				t_pos		= Input.touches[0].position;
			}
#else
			t_clicked 	= Input.GetMouseButtonDown(0);
			if(t_clicked){
...
/// <summary>
/// Sea client color type.
/// </summary>
public enum SeaClientColorType
{
	// Nothing color to use original color;
	NothingColor	= 0x0DFDFDCC,

    FloralWhite247246220 = 0xFEF6DC, // 254 246 220
    //NewColor2H = 0x000000, // 0 0 0
    Red255000000 = 0xFF0000, // 255 0 0
    Green000255000 = 0x00FF00, // 0 255 0
    Gold254221001 = 0xFEDD01, // 254 221 1
    Purple219039252 = 0xDB27FC, // 219 39 252
    Blue014213249 = 0x0ED5F9, // 14 213 249
	
	DarkRed210000005 = 0xD20005, // 210 0 5
	LimeGreen089210000 = 0x59D200, // 89 210 0
	SeaGreen068199120 = 0x44C778, // 68 199 120
	Tan188149067 = 0xBC9543, // 188 149 67
	Cyan000255204 = 0x00FFCC, // 0 255 204
	
	LightBlueSky000216255 = 0x00D8FF, // 0 216 255
	
	White255255255 = 0xFFFFFF, // 255 255 255
	Black000000000 = 0x000000, // 0 0 0
	Gray182182182 = 0xB6B6B6, // 182 182 182
}
