...
public class RouteEngine
{
    ...
    public Dictionary<Location, Route> CalculateMinCost(Location _startLocation)
    {
		...
		Debug.Log("location count:"+_locations.Count);
        ...
        while (_handledLocations.Count != _locations.Count)
        {
			...
			List<KeyValuePair<Location, Route>> myList = new List<KeyValuePair<Location, Route>>();
			foreach (KeyValuePair<Location,Route> r in _shortestPaths)
			{
				myList.Add(r);
			}
		
		    myList.Sort(
		        delegate(KeyValuePair<Location, Route> firstPair,
		        KeyValuePair<Location, Route> nextPair)
		        {
		            return firstPair.Value.Cost.CompareTo(nextPair.Value.Cost);
		        }
		    );
			List<Location> _shortestLocations = new List<Location>();
			foreach(KeyValuePair<Location,Route> s in myList)
			{
				_shortestLocations.Add(s.Key);
			}
...
//			
			...
			List<Connection> _selectedConnections = new List<Connection>();
			foreach( Connection c in _connections)
			{
				if(c.A == _locationToProcess)
