...
public class MyCenterOnChild : UICenterOnChild
{
	/// <summary>
...
//	void way()
//	{
//		iTween.RotateTo(mCenteredObject,new Vector3(0f,0f,0f),0.5f);
//	}
    void CenterOn (Transform target, Vector3 panelCenter)
	{
		if (target != null && mScrollView != null && mScrollView.panel != null)
		{
			...
			if(mcenter != null && mcenter != mCenteredObject)
			{
				
			iTween.ScaleTo(mcenter,new Vector3(1f,1f,1f),0.5f);
			iTween.RotateTo(mcenter,new Vector3(0f,0f,0f),1f);
//			mcenter.transform.localScale = new Vector3(1f, 1f, 1.0f);
			}
			int a;
			
//			if(times > 0)//第一次不执行//
//			{
//			iTween.RotateTo(mCenteredObject,new Vector3(0f,180f,0f),0.5f);
//			Invoke("way",0.5f);
			
//			}
//			times++;
			iTween.ScaleTo(mCenteredObject,new Vector3(1.35f,1.35f,1.0f),0.5f);
//			   mCenteredObject.transform.localScale = new Vector3(1.35f, 1.35f, 1.0f); //处于中间位置的对象放大显示// 
			mcenter = mCenteredObject;
