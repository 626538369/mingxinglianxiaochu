...
public class AI : MonoBehaviour
{
	...
	{
		return EliminationMgr.Instance.GetSquare (col, row);
	}
...
//find possible combination again 
	...
	IEnumerator showTipCor (List<Item> nextMoveItems)
	{
		...
		if (EliminationMgr.Instance.DragBlocked && !allowShowTip) {
			corCount--;
			yield break;
		}
		tipID = EliminationMgr.Instance.moveID;
		//while (!LevelManager.THIS.DragBlocked && allowShowTip)
		//{
		yield return new WaitForSeconds (1);
		if (EliminationMgr.Instance.DragBlocked && !allowShowTip && tipID != EliminationMgr.Instance.moveID) {
			corCount--;
			yield break;
		}
