...
public class HQGeneralTipsContent : TipsContent
{
	//----------------------------------------------
	...
	public override void SetValue(params object[] args)
	{
		...
		_mTipsData = (HQGeneralData)args[0];
		if (null == _mTipsData)
			return;
		if (null == _mTipsData.GeneralData)
			return;
		
		BuildTips(_mTipsData.GeneralData);
		CalculatePosition();
	}
	...
	{
		string val = GUIFontColor.Orange + data.BasicData.Name;
		return val;
	}
	...
	{
		string wordText = Globals.Instance.MDataTableManager.GetWordText(20700006);
		string val = GUIFontColor.Orange + wordText;
		foreach (SkillData sdata in data.SkillDatas.Values)
		{
			val += sdata.BasicData.SkillName + "\n";
			break;
		}
	...
	private HQGeneralData _mTipsData;
}
